import { useEffect, useRef, type CSSProperties, type CanvasHTMLAttributes } from 'react';
import './GlyphMatrix.css';

export interface GlyphMatrixProps extends CanvasHTMLAttributes<HTMLCanvasElement> {
  ...
  fadeBottom?: number;
  /** Glyph color (any CSS color). Defaults to BemoUI blue. */
  color?: string;
  className?: string;
  style?: CSSProperties;
}
...
export function GlyphMatrix({
  ...
  const classes = ['bemo-glyph-matrix', className].filter(Boolean).join(' ');
