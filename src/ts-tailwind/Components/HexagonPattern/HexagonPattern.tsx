...
export default function HexagonPattern({
  ...
  const rootClass = [
    'pointer-events-none absolute inset-0 h-full w-full fill-[#1620E4]/30 stroke-[#1620E4]/30 motion-reduce:transition-none',
    className,
  ]
    .filter(Boolean)
    .join(' ');
      ...
      {hexagons && hexagons.length > 0 && (
        ...
        >
          {hexagons.map(([col, row]) => {
            ...
            return (
              <polygon
                className="fill-[#7BE9C6]/45 stroke-0"
                key={`${col}-${row}`}
...
/* Tailwind v4 global keyframes: none required — HexagonPattern is a static SVG pattern. */
