...
function DefaultOrb({ index, iconSize }) {
  const isAccent = index % 2 === 1;
  return (
    <span
      aria-hidden="true"
      className={`inline-flex items-center justify-center rounded-full font-semibold leading-none shadow-md ${
        isAccent
          ? 'bg-[#7BE9C6] text-neutral-900 shadow-[0_4px_14px_rgba(123,233,198,0.35)]'
          : 'bg-[#1620E4] text-white shadow-[0_4px_14px_rgba(22,32,228,0.28)]'
      }`}
...
export function IconCloud({
  ...
  const handlePointerDown = (event) => {
    ...
    positions.forEach((icon) => {
      ...
      if (dx * dx + dy * dy <= radiusHit * radiusHit) {
        if (!hit || p.z > hit.z) hit = { icon, z: p.z };
      }
    });
  ...
  return (
    <div
      ...
      className={`relative inline-block overflow-hidden rounded-xl bg-transparent touch-none select-none outline-none perspective-[900px] focus-visible:ring-2 focus-visible:ring-[#1620E4] focus-visible:ring-offset-2 ${
        disabled ? 'cursor-not-allowed opacity-60' : isDragging ? 'cursor-grabbing' : 'cursor-grab'
      } ${className}`}
      style={{ width: size, height: size }}
    >
      <div className="absolute inset-0 [transform-style:preserve-3d] pointer-events-none" aria-hidden="true">
        {sorted.map(({ icon, index, projected }) => (
          <div
            key={icon.id}
            className="absolute top-1/2 left-1/2 flex items-center justify-center m-0 pointer-events-none [transform-style:preserve-3d] -translate-x-1/2 -translate-y-1/2 motion-reduce:transition-none"
            style={{
              width: iconSize,
              height: iconSize,
...
/*
  ...
  Rotation is driven by requestAnimationFrame and respects prefers-reduced-motion.
  Optional utility if you prefer pure CSS idle spin instead of JS:
  @keyframes bemo-icon-cloud-spin {
