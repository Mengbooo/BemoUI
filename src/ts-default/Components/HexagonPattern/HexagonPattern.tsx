...
export interface HexagonPatternProps extends SVGProps<SVGSVGElement> {
  ...
  y?: number;
  /** Hexagon orientation. @default "horizontal" */
  direction?: 'horizontal' | 'vertical';
  /** SVG stroke-dasharray applied to each hexagon outline. @default "0" */
  strokeDasharray?: string;
  /** [col, row] coordinates for highlighted hexagons. */
...
export default function HexagonPattern({
  ...
  const rootClass = ['bemo-hexagon-pattern', className].filter(Boolean).join(' ');
    ...
    >
      <defs>
        ...
        >
          {solidStroke
            ? centers.map(([cx, cy]) => (
                <polygon
                  className="bemo-hexagon-pattern__hex-outline"
                  key={`${cx}-${cy}`}
            ...
            : dashedEdges?.map(([a, b]) => (
                <line
                  className="bemo-hexagon-pattern__edge"
                  key={edgeLexKey(a, b)}
                  x1={a[0]}
                  x2={b[0]}
                  y1={a[1]}
      ...
      {hexagons && hexagons.length > 0 && (
        <svg
          aria-hidden="true"
          focusable="false"
          className="bemo-hexagon-pattern__highlight-layer"
          x={x}
          y={y}
        >
          {hexagons.map(([col, row]) => {
            ...
            return (
              <polygon
                className="bemo-hexagon-pattern__highlight"
                key={`${col}-${row}`}
