...
import './TwentyFirstCultDynamicIsland.css';
...
export const DynamicIslandProvider: React.FC<DynamicIslandProviderProps> = ({
  ...
  const contextValue: BlobContextType = {
    state,
    dispatch,
    setSize,
    scheduleAnimation,
...
const DynamicIslandContainer: React.FC<DynamicIslandContainerProps> = ({
  children,
  className = '',
}) => {
  return (
    <div
      className={`bemo-21st-cult-dynamic-island-container ${className}`.trim()}
    >
      {children}
    </div>
  );
...
const DynamicIslandContent: React.FC<DynamicIslandContentProps> = ({
  ...
  return (
    <motion.div
      ...
      role="region"
      aria-live="polite"
      className={`bemo-21st-cult-dynamic-island ${className}`.trim()}
      animate={{
...
export const DynamicContainer: React.FC<DynamicContainerProps> = ({
  ...
  return (
    <motion.div
      ...
      style={{ willChange }}
      className={`bemo-21st-cult-dynamic-island-container-inner ${className}`.trim()}
    >
      {children}
    </motion.div>
  );
...
export const DynamicDiv: React.FC<DynamicChildrenProps> = ({
  ...
  return (
    <motion.div
      ...
      style={{ willChange }}
      className={`bemo-21st-cult-dynamic-island-div ${className}`.trim()}
    >
      {children}
    </motion.div>
  );
...
export const DynamicTitle: React.FC<MotionTextProps> = ({
  className = '',
  children,
}) => {
  const { state } = useDynamicIslandSize();
  ...
  return (
    <motion.h3
      className={`bemo-21st-cult-dynamic-island-title ${className}`.trim()}
...
export const DynamicDescription: React.FC<MotionTextProps> = ({
  ...
  return (
    <motion.p
      className={`bemo-21st-cult-dynamic-island-description ${className}`.trim()}
      initial={{ opacity: 0, scale: 0 }}
      animate={{
        opacity: size === previousSize ? 0 : 1,
        scale: size === previousSize ? 0.9 : 1,
