...
function TwentyFirstCultShaderLensBlur({
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      defines: {
        VAR: Math.max(0, Math.min(3, Math.floor(variation))),
      },
      transparent: true,
  ...
  useEffect(() => {
    ...
    const nextVar = Math.max(0, Math.min(3, Math.floor(variation)));
  ...
  return (
    <motion.div
      ref={containerRef}
      className={`relative block overflow-hidden rounded-lg bg-black touch-none select-none outline-none isolation-isolate focus-visible:ring-2 focus-visible:ring-[#1620E4] focus-visible:ring-offset-2 focus-visible:ring-offset-white disabled:opacity-55 disabled:pointer-events-none disabled:grayscale-[0.35] ${disabled ? 'cursor-not-allowed' : ''} ${className}`}
    ...
    >
      <canvas
        ref={canvasRef}
        className="block size-full touch-none align-middle"
        aria-hidden="true"
      />
      {showHint && (
        <div
          className="pointer-events-none absolute bottom-4 left-4 right-4 m-0 p-0 text-sm leading-snug text-neutral-300 max-w-[calc(100%-2rem)] [text-shadow:0_1px_2px_rgba(0,0,0,0.65)] sm:bottom-3 sm:left-3 sm:right-3 sm:text-xs"
          aria-hidden="true"
        >
          {hintText}
        </div>
...
/* Tailwind v4 keyframes (none required for this shader component) */
