...
const DefaultLogo: React.FC<{ name?: string }> = ({ name }) => (
  <div
    ...
    const renderLogo = () => {
      ...
      if (React.isValidElement(currentLogo.img)) {
        return React.cloneElement(currentLogo.img as React.ReactElement<{ className?: string; 'aria-hidden'?: boolean }>, {
          className: `block h-20 w-20 max-h-[80%] max-w-[80%] object-contain text-black md:h-32 md:w-32 ${(currentLogo.img as React.ReactElement<{ className?: string }>).props?.className || ''}`.trim(),
          'aria-hidden': true,
        });
      }
      if (currentLogo.src) {
