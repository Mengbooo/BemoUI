...
const CharBox = memo(function CharBox({
  ...
  return (
    <span
      className="bemo-text-3d-flip-char inline [transform-style:preserve-3d]"
      style={{ transform: CONTAINER_TRANSFORMS[rotateDirection] }}
      aria-hidden="true"
    >
      <span
        className={joinClass(
          'relative block h-[1lh] [backface-visibility:hidden] text-[#1620E4]',
          textClassName
        )}
      ...
      <span
        className={joinClass(
          'absolute top-0 left-0 block h-[1lh] [backface-visibility:hidden] text-[#7BE9C6]',
          flipTextClassName
        )}
...
const Text3DFlip = ({
  ...
  return (
    <ElementTag
      ref={scope}
      className={joinClass(
        'relative flex flex-wrap m-0 text-[#1620E4] outline-none focus-visible:outline-2 focus-visible:outline-[#1620E4] focus-visible:outline-offset-4 rounded-sm',
...
/* Tailwind v4 global keyframes: none required.
   Character 3D flip timing is driven by framer-motion springs (useAnimate).
   Prefer CSS prefers-reduced-motion handling via matchMedia in the component.
   Optional utility if your setup lacks sr-only:
   @utility sr-only {
     position: absolute;
     width: 1px;
     height: 1px;
