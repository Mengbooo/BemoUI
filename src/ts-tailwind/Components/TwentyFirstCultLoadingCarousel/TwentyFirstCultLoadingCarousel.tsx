...
export interface TwentyFirstCultLoadingCarouselProps {
  tips?: Tip[];
  className?: string;
...
function getTipKey(tip: Tip, index: number): string {
...
const aspectRatioClass: Record<NonNullable<TwentyFirstCultLoadingCarouselProps['aspectRatio']>, string> = {
  video: 'aspect-video',
...
export function TwentyFirstCultLoadingCarousel({
  ...
  const [displayTips] = useState<Tip[]>(() => (shuffleTips ? shuffleArray(tips) : tips));
