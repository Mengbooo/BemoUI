...
function TwentyFirstDock({
  ...
  return (
    <motion.div
      ...
      className={cn('mx-2 flex max-w-full items-end overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden', className)}
      {...rest}
    >
      <motion.div
        onMouseMove={({ pageX }) => {
...
function DockIcon({ children, className, ...rest }) {
  ...
  return (
    <motion.div
      ...
      className={cn('flex items-center justify-center text-neutral-900 dark:text-neutral-50 [&_svg]:w-full [&_svg]:h-full [&_svg]:max-w-6 [&_svg]:max-h-6', className)}
    >
      {children}
    </motion.div>
  );
