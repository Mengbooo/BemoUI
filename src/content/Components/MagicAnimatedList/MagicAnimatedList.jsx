...
import './MagicAnimatedList.css';

function MagicAnimatedListItem({ children, reduceMotion }) {
  ...
  return (
    <motion.div
      {...animations}
      layout={!reduceMotion}
      className="bemo-animated-list__item"
      role="listitem"
    >
      {children}
    </motion.div>
...
const MagicAnimatedList = React.memo(function MagicAnimatedList({
  ...
  return (
    <div
      className={['bemo-animated-list', className].filter(Boolean).join(' ')}
      role="list"
      {...props}
    >
